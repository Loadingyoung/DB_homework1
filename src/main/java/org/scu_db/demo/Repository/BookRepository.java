...
public interface BookRepository extends JpaRepository<Book, Integer> {
    ...
    @Query(value = "select count(book_id) from book where callnumber = ?1 group by callnumber", nativeQuery = true)
    Integer countBooksByCallnumber(String callnumber);

    @Query(value = "select callnumber from book group by callnumber having count(book_id) <= ?1", nativeQuery = true)
